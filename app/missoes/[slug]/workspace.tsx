...
import { BattleActions, BattleHeader, BattlePanel, type BattleAction, type BattleFeedback, type BattleResultItem, type BattleView } from "./battle-card";
...
type Submission = { ok: boolean; compiled?: boolean; message: string; results?: BattleResultItem[]; gainedXp?: number; totalXp?: number; unlockedSlug?: string | null; battle?: { lives: number; state: BattleView["state"]; hint?: string } | null };
...
export function MissionWorkspace({ mission, initialBattle }: { mission: MissionView; initialBattle?: BattleView }) {
  ...
  return <div className={`battle-screen${feedback ? ` hit-${feedback}` : ""}`} data-testid="battle-workspace">
    ...
    <div className="battle-ide-layout no-preview">
      {battle ? <BattlePanel battle={battle} technology={mission.technologyName} objective={mission.objective} results={results} hint={hint} feedback={feedback} loading={loading} onRevive={() => submit("revive")} /> : null}
      ...
      <BattleActions battle={battle} loading={loading} onAction={submit} />
    </div>
  </div>;
}
