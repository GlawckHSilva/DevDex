...
export function MissionWorkspace({ mission }: { mission: MissionView }) {
  ...
  return <div className="workspace-grid">
    <aside className="briefing-panel"><Link href="/dashboard">← Voltar às missões</Link><span className="kicker">BRIEFING</span><h1>{mission.title}</h1><p>{mission.briefing}</p><div className="objective"><small>OBJETIVO</small><p>{mission.objective}</p></div><div className="rules"><small>AMBIENTE SEGURO</small><p>Esta missão aceita uma função e uma expressão de retorno. Operações fora da lista segura são recusadas.</p></div></aside>
    <section className="editor-panel">
      ...
      <Editor height="100%" language="javascript" theme="vs-dark" value={code} onChange={(value) => setCode(value ?? "")} options={{ automaticLayout: true, fontSize: 14, fontFamily: "var(--font-geist-mono)", minimap: { enabled: false }, padding: { top: 18 }, scrollBeyondLastLine: false, tabSize: 2, wordWrap: "on" }} />
    </section>
