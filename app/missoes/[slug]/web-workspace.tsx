...
export function WebWorkspace({ mission }: { mission: WebMission }) {
  ...
  return <div className="web-workspace-grid">
    <aside className="briefing-panel web-briefing"><Link href={`/trilhas/${path}`}>← Voltar à trilha</Link><span className="kicker">MISSÃO · {mission.documentType.toUpperCase()}</span><h1>{mission.title}</h1><p>{mission.briefing}</p><div className="objective"><small>OBJETIVO</small><p>{mission.objective}</p></div><div className="rules"><small>PREVIEW ISOLADO</small><p>Scripts, rede, navegação e acesso à aplicação permanecem bloqueados.</p></div></aside>
