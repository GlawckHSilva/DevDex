...
function previewDocument(mission: WebMission, code: string) {
  ...
  if (mission.documentType === "html" && /<head(?:\s[^>]*)?>/i.test(html)) return html.replace(/<head(\s[^>]*)?>/i, `<head$1>${head}`);
...
export function WebWorkspace({ mission, initialBattle }: { mission: WebMission; initialBattle?: BattleView }) {
  const initialCode = mission.documentType === "html" ? HTML_DOCUMENT_STARTER : mission.starterCode;
  const { value: code, setValue: setCode, reset: resetCode } = useMissionDraft(mission.slug, initialCode, mission.starterCode);
