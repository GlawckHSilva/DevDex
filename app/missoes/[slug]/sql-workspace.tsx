...
type Submission = { ok: boolean; message: string; columns?: string[]; rows?: SqlValue[][]; results?: BattleResultItem[]; gainedXp?: number; newlyCompleted?: boolean; unlockedSlug?: string | null; levelUp?: { fromLevel: number; toLevel: number; skillPointsGained: number } | null; guidance?: string | null; battle?: Partial<BattleView> & { lives: number; state: BattleView["state"]; hint?: string } };
...
export function SqlWorkspace({ mission, initialBattle }: { mission: SqlMission; initialBattle?: BattleView }) {
  const { value: query, setValue: setQuery, reset: resetQuery } = useMissionDraft(mission.slug, mission.starterSql);
  ...
  return <div className={`battle-screen${feedback ? ` hit-${feedback}` : ""}`} data-testid="battle-workspace">
    ...
    <BattleTabs className="battle-sql-layout">
      <BattleTabList />
