...
export function SqlWorkspace({ mission }: { mission: SqlMission }) {
  ...
  return <div className="sql-workspace-grid">
    <aside className="briefing-panel sql-briefing"><a href="/trilhas/sql-fundamentals-sqlite">← Voltar à trilha</a><span className="kicker">MISSÃO · {mission.dialect.toUpperCase()}</span><h1>{mission.title}</h1><p>{mission.briefing}</p><div className="objective"><small>OBJETIVO</small><p>{mission.objective}</p></div><div className="rules"><small>BANCO DESCARTÁVEL</small><p>O banco nasce do zero nesta execução e é apagado ao final. Somente um SELECT é permitido.</p></div></aside>
