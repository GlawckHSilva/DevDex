...
export function ProjectWorkspace({ project }: { project: Project }) {
  ...
  return <div className="project-workspace-grid">
    ...
    <section className="project-tests"><div className="console-actions"><span>REQUISITOS · {currentStep.xpReward} XP</span><div><button className="button button-ghost" disabled={loading !== null} onClick={() => submit("run")}>{loading === "run" ? "Executando…" : "▷ Executar"}</button><button className="button" disabled={loading !== null || project.state === "completed"} onClick={() => submit("test")}>{loading === "test" ? "Validando…" : "✓ Validar etapa"}</button></div></div><div className="project-test-body" aria-live="polite">{submission?.projectCompleted ? <div className="project-complete"><span>🏆 PROJETO CONCLUÍDO</span><strong>To-do App</strong><a href="/dashboard">Ver no dashboard →</a></div> : <>{(submission?.results ?? currentStep.requirements.map((name) => ({ name, passed: false }))).map((result) => <p className={result.passed ? "passed" : ""} key={result.name}>{result.passed ? "✓" : "○"} {result.name}</p>)}{submission ? <p className={submission.ok ? "console-success" : "console-error"}>{submission.message}</p> : null}{submission?.ok && submission.nextStepSlug ? <button className="next-step" onClick={() => router.refresh()}>Avançar para a próxima etapa →</button> : null}</>}</div></section>
  </div>;
}
