...
type Repository = { repositoryUrl: string; branch: string; latestCommitSha: string | null; reviewStatus: "linked" | "passed" | "needs_changes" | "error"; passedTests: number; failedTests: number; reviewedAt: string | null; aiStatus: "unavailable" | "completed" | "error"; aiSummary: string | null; aiStrengths: string[]; aiImprovements: string[]; aiNextStep: string | null };
type Project = { slug: string; title: string; description: string; introduction: string; deadlineDays: number; state: "available" | "in_progress" | "completed"; completedSteps: number; files: FileItem[]; steps: Step[]; repository: Repository | null; githubConnected: boolean; githubAppEnabled: boolean; aiEnabled: boolean };
...
export function ProjectWorkspace({ project, backHref = "/dashboard" }: { project: Project; backHref?: string }) {
  ...
  return <div className="project-workspace-grid">
    ...
    <section className="preview-panel project-preview"><div className="preview-bar"><span>PREVIEW</span><small>SANDBOX ISOLADO</small></div><iframe title="Preview do projeto" sandbox="allow-scripts" referrerPolicy="no-referrer" srcDoc={preview} /></section>
      <section className="project-tests"><div className="console-actions"><span>REQUISITOS · {currentStep.xpReward} XP</span><div><button className="button button-ghost" disabled={loading !== null} onClick={() => submit("run")}>{loading === "run" ? "Executando…" : "▷ Executar"}</button><button className="button" disabled={loading !== null || project.state === "completed"} onClick={() => submit("test")}>{loading === "test" ? "Validando…" : "✓ Validar etapa"}</button></div></div><RepositoryReview projectSlug={project.slug} initial={project.repository} githubConnected={project.githubConnected} githubAppEnabled={project.githubAppEnabled} aiEnabled={project.aiEnabled} /><div className="project-test-body" aria-live="polite">{submission?.projectCompleted ? <div className="project-complete"><span>🏆 PROJETO CONCLUÍDO</span><strong>{project.title}</strong><a href="/dashboard">Ver no dashboard →</a></div> : <>{(submission?.results ?? currentStep.requirements.map((name) => ({ name, passed: false }))).map((result) => <p className={result.passed ? "passed" : ""} key={result.name}>{result.passed ? "✓" : "○"} {result.name}</p>)}{submission ? <p className={submission.ok ? "console-success" : "console-error"}>{submission.message}</p> : null}{submission?.ok && submission.nextStepSlug ? <button className="next-step" onClick={() => router.refresh()}>Avançar para a próxima etapa →</button> : null}</>}</div></section>
  </div>;
}
