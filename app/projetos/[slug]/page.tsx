...
import { getProject, getProjectRepository, hasGitHubInstallation } from "@/db";
...
export default async function ProjectPage({ params, searchParams }: { params: Promise<{ slug: string }>; searchParams: Promise<{ campaign?: string }> }) {
  ...
  const [repository, githubConnected] = await Promise.all([getProjectRepository(user.userId, project.id), hasGitHubInstallation(user.userId)]);
