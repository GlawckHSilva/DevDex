...
export default async function CampaignPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main className={`game-campaign-shell theme-${campaign.theme}`} id={campaign.pathSlug}>
    <section className="game-campaign-main"><nav className="game-campaign-topbar" aria-label="Navegação da campanha"><a className="brand" href="/"><span className="brand-mark">D_</span>DevDex</a><a href="/dashboard">← Voltar às campanhas</a></nav><header className="game-campaign-header"><div className="campaign-title-block"><span className="kicker">CURSO COMPLETO · {campaign.technologyName}</span><h1>{campaign.title}</h1><p>{campaign.storyIntro}</p><small className="course-format">30 aulas explicativas · 30 práticas · básico ao profissional</small></div><div className="header-campaign-progress"><span>PROGRESSO DO CURSO <b>{campaign.progress}%</b></span><div className="progress-track"><i style={{ width: `${campaign.progress}%` }} /></div><small>{completedMissions} / {totalMissions} missões concluídas</small></div>{character ? <div className="game-player-card"><PixelHero archetype={character.archetype} small /><div><strong>{user.displayName.split("@")[0]}</strong><small>NÍVEL {profile.level}</small><b>{profile.totalXp} XP</b></div></div> : null}{character ? <div className="game-hero-large"><PixelHero archetype={character.archetype} /><small>AVENTUREIRO DEVDEX</small></div> : null}</header>
